...
contract AcademicChain is AccessControl {
    ...
    function addStudent(string memory _name, uint8 _age, address payable _wallet) public onlyAdmin {
    ...
    function updateStudent(uint256 _studentId, string memory _name, uint8 _age, address payable _wallet)
        public
        onlyAdmin
    {
    ...
    function addCourse(uint256 _studentId, string memory _name, uint8 _credits, uint8 _grade) public onlyAdmin {
        require(students[_studentId].exists, "Student does not exist");
        ...
        Student storage student = students[_studentId];
        student.courses.push(Course({name: _name, credits: _credits, grade: _grade}));
    ...
    function getStudentData(uint256 _studentId)
        public
        view
        returns (string memory name, uint8 age, address wallet, Course[] memory courses, bool exists)
    {
        require(students[_studentId].exists, "Student does not exist");
